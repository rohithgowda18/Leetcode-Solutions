    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        if (url === '/api/git/status' && req.method === 'GET') {
          try {
            const { getGitRemoteStatus } = await import('./server/api');
            const status = getGitRemoteStatus();
            res.setHeader('Content-Type', 'application/json');
        ...
        // 7. POST /api/git/config
        if (url === '/api/git/config' && req.method === 'POST') {
          let body = '';
          req.on('data', chunk => { body += chunk; });
          req.on('end', async () => {
            try {
              const { configureGitRemote } = await import('./server/api');
              const data = JSON.parse(body);
              const result = configureGitRemote(data);
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify(result));
            } catch (err: any) {
              res.statusCode = 500;
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ error: err.message || 'Failed to configure remote' }));
            }
          });
          return;
        }
        ...
        if (url === '/api/git/sync' && req.method === 'POST') {
          ...
          req.on('end', async () => {
            try {
              const { syncGitRemote } = await import('./server/api');
              const data = body ? JSON.parse(body) : {};
              const result = syncGitRemote(data);
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify(result));
            } catch (err: any) {
              res.statusCode = 500;
              res.setHeader('Content-Type', 'application/json');
              res.end(JSON.stringify({ error: err.message || 'Git sync failed' }));
            }
          });
          return;
        }
