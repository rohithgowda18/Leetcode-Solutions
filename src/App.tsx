  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col items-center py-8 px-4 sm:px-6 selection:bg-amber-500/30 selection:text-amber-200">
      ...
      <main className="w-full max-w-4xl space-y-6">
        {/* Git Repository Sync Banner */}
        {showGitSync && (
          <div className="bg-neutral-900/90 border border-neutral-800 rounded-2xl p-5 shadow-xl backdrop-blur-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center justify-center text-amber-400 font-bold shadow-inner shrink-0">
                  <GitFork className="w-5 h-5 text-amber-400" />
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-base font-bold text-neutral-100">Git Repository Sync</h2>
                    <span
                    ...
                    >
                      {hasRemoteConfigured ? 'Remote Configured' : 'No Remote URL'}
                    </span>
                  </div>
                  <p className="text-xs text-neutral-400">
                    Configure a local or remote Git URL to sync your organized solutions
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 self-start sm:self-auto">
                <button
                  type="button"
                  onClick={() => setShowGitSync(false)}
                  className="px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-neutral-300 text-xs rounded-xl transition-colors flex items-center gap-1.5 cursor-pointer"
                >
                ...
                <button
                  ...
                  className="px-4 py-1.5 bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-neutral-950 font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 cursor-pointer shadow-md"
                >
                  {isSyncingGit ? (
                    <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
              <div className="sm:col-span-2 space-y-1.5">
                <label className="text-xs font-semibold text-neutral-300">Git Remote URL</label>
                <input
                  type="text"
                  placeholder="https://github.com/username/leetcode-solutions.git"
                  value={gitRemoteUrl}
                  onChange={e => setGitRemoteUrl(e.target.value)}
                  className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3.5 py-2 text-xs text-neutral-100 font-mono focus:border-amber-500/60 focus:ring-1 focus:ring-amber-500/30 outline-none"
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-semibold text-neutral-300">Target Branch</label>
                <input
                  type="text"
                  placeholder="main"
                  value={gitBranch}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-semibold text-neutral-300">Custom Commit Message (Optional)</label>
              <input
                type="text"
                placeholder="e.g., Add Two Sum and Combination Sum Java solutions"
                value={customCommitMsg}
                onChange={e => setCustomCommitMsg(e.target.value)}
                className="w-full bg-neutral-950 border border-neutral-800 rounded-xl px-3.5 py-2 text-xs text-neutral-100 placeholder:text-neutral-600 focus:border-amber-500/60 focus:ring-1 focus:ring-amber-500/30 outline-none"
              />
            </div>
            ...
            {gitSyncStatusMsg && (
              <div
                className={`p-2.5 rounded-xl text-xs flex items-center gap-2 ${
                  gitSyncStatusMsg.type === 'success'
                    ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20'
                    : gitSyncStatusMsg.type === 'error'
              ...
              >
                ...
                <span>{gitSyncStatusMsg.text}</span>
              </div>
            )}

            {/* Bottom Footer Action */}
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 pt-1 border-t border-neutral-800/60 text-[11px] text-neutral-500">
              <div className="flex items-center gap-1.5">
                <ShieldCheck className="w-3.5 h-3.5 text-neutral-400" />
                <span>Pushes via system-level credentials (SSH keys or Git Credential Manager).</span>
              </div>
              <button
                type="button"
                onClick={handleSaveGitConfig}
                className="px-3 py-1 bg-neutral-800 hover:bg-neutral-700 text-neutral-200 font-semibold rounded-lg transition-colors cursor-pointer self-end sm:self-auto"
              >
                Save Remote Config
              </button>
            </div>
